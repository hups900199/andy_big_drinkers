class ImagesController < ApplicationController
  ...
  def new_image
    @images = Image.includes(:anime).includes(:types).where("images.created_at > ?",
                                                            3.days.ago).order("images.name ASC").page params[:page]
  ...
  def recent_update
    @images = Image.includes(:anime).includes(:types).where("images.created_at < ?", 3.days.ago).where(
      "images.updated_at > ?", 3.days.ago
    ).order("images.name ASC").page params[:page]
